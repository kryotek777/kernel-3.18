...
//Jorge 2015.07.25 discharge temperature is 60
//	if (BMT_status.temperature >= batt_cust_data.max_charge_temperature) {
	if ((BMT_status.charger_exist && (BMT_status.temperature >= batt_cust_data.max_charge_temperature)) || (BMT_status.temperature >= 60)){
...
/* Jorge 2015.05.13 Acer need check power off voltage in factory mode or recovery mode */
static void mt_battery_poweroff_voltage_check(void)
{
	if (BMT_status.bat_vol < SYSTEM_OFF_VOLTAGE) 
	{
		if ((g_platform_boot_mode == META_BOOT) || (g_platform_boot_mode == NORMAL_BOOT)
		|| (g_platform_boot_mode == ADVMETA_BOOT) || (g_platform_boot_mode == SW_REBOOT)
		|| (g_platform_boot_mode == ATE_FACTORY_BOOT) || (g_platform_boot_mode == ALARM_BOOT)
		|| (g_platform_boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT) || (g_platform_boot_mode == LOW_POWER_OFF_CHARGING_BOOT)) 
		{
			battery_log(BAT_LOG_FULL,
				"[BATTERY] boot mode = %d, bypass poweroff voltage check\n",
				g_platform_boot_mode);
		} 
		else 
		{
			...
			battery_log(BAT_LOG_CRTI,
				"[Battery] bat_vol(%d) < %d, system need power down.\n",
...
//Jorge 2015.12.04 add close Red Led Start
static int power_on_times = 0;
...
//Jorge 2015.12.04 add close Red Led Start
		if (power_on_times < 30) // 5 Minutes
		{
		    pmic_config_interface(0x010E, 0x0, 0x1, 1); 
		    pmic_config_interface(0x0126, 0x0, 0x1, 11); 
	 	    pmic_config_interface(0x0338, 0xF, 0x1F, 8); 
		    pmic_config_interface(0x033A, 0x0, 0xFFFF, 0); 
		    pmic_config_interface(0x0330, 0x0, 0x3, 2); 
		    pmic_config_interface(0x033C, 0x3, 0x7, 12); 
		    pmic_config_interface(0x0356, 0x0, 0x1, 5); 

		    pmic_config_interface(0x0356, 0x0, 0x1, 1); 

			power_on_times++;			
		}
//Jorge 2015.12.04 add close Red Led End
...
void BAT_thread(void)
{
	...
	/* Jorge 2015.05.13 Acer need check power off voltage in factory mode or recovery mode */
	mt_battery_poweroff_voltage_check();
}
