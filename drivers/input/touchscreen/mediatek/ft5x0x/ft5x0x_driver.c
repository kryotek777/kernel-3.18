...
static ssize_t write_proc(struct file *file, const char *buf, size_t count, loff_t *data)
{
	...
	if (count == 8) {
		...
		TPD_DMESG("---hxh write_proc mIsEnableGestureWakeUp = %d\n",mIsEnableGestureWakeUp);
		TPD_DMESG("---hxh write_proc mIsEnabletwofinger = %d\n",mIsEnabletwofinger);
		TPD_DMESG("---hxh write_proc mIsEnablefivefinger = %d\n",mIsEnablefivefinger);
		TPD_DMESG("---hxh write_proc mIsEnableDoubleTab = %d\n",mIsEnableDoubleTab);
...
static ssize_t store_scp_ctrl(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (kstrtoul(buf, 10, &cmd)) {
		TPD_DMESG("[SCP_CTRL]: Invalid values\n");
		return -EINVAL;
	}
	...
	case 5:
		{
			...
			if (md32_ipi_send(IPI_TOUCH, &ipi_pkt, sizeof(ipi_pkt), 0) < 0)
				TPD_DMESG("[TOUCH] IPI cmd failed (%d)\n", ipi_pkt.cmd);

			break;
		}
	default:
	    TPD_DMESG("[SCP_CTRL] Unknown command");
	    break;
	}
...
//		TPD_DMESG("tpd x=%d\n",x);
//		TPD_DMESG("tpd y=%d\n",y);	
//		TPD_DMESG("tpd ABS_MT_POSITION_X=%d\n",ABS_MT_POSITION_X);
//		TPD_DMESG("tpd ABS_MT_POSITION_Y=%d\n",ABS_MT_POSITION_Y);
	}
}
...
static int tpd_touchinfo(struct touch_info *cinfo, struct touch_info *pinfo)
{
    ...
    i2c_smbus_read_i2c_block_data(i2c_client, 0x38, 8,  &(data[56]));
	...
	if ((data[0] & 0x70) != 0)
	{
        TPD_DMESG("hxh Device Mode[2:0] != 0...\n");
		return false;
	    }
